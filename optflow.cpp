...
int main() {
	...
	Mat opticalFlow = Mat(cap.get(CAP_PROP_FRAME_HEIGHT),
		cap.get(CAP_PROP_FRAME_HEIGHT), CV_32FC3);
	...
	while (1) {
		...
		cvtColor(rgbFrames, grayFrames, COLOR_BGR2GRAY);
...
//		imshow(opticalFlowWindow, opticalFlow);
		...
		if (keyPressed == 27) {
			...
			opticalFlow = Mat(cap.get(CAP_PROP_FRAME_HEIGHT),
				cap.get(CAP_PROP_FRAME_HEIGHT), CV_32FC3);
		}
